((Drupal, once) => {
  ...
  Drupal.behaviors.pageSocialShare = {
    ...
    attach(context) {
      once('init-offcanvas', `#${this.burgerButtonsSelector}`, context).forEach(
        (button) => {
    ...
    handleBodyClick(event) {
      ...
      const isBackdrop =
        !event.target.closest(`#${this.offcanvasMenuSelector}`) &&
    ...
    toggleOffcanvasSocial(willOpen) {
      const $offcanvasMenu = document.getElementById(
        this.offcanvasMenuSelector,
      );
      const $burgerButtons = document.getElementById(
        this.burgerButtonsSelector,
      );
...
((Drupal, once) => {
   ...
   * @type {Drupal.behavior}
   */
  Drupal.behaviors.pageAttributionBubble = {
    // Define Drupal Behaviour
    ...
    attach(context) {
      ...
      once(
        'init-attribution-bubble',
        this.attributionBubbleToggleSelector,
        context,
      ).forEach((attributionBubbleToggle) => {
      ...
      once('init-attribution-bubble-backdrop', 'body', context).forEach(
        (body) => {
    ...
    attributionBubbleToggle(event) {
      ...
      const attributionBubbleToggleElement =
        attributionBubbleElement.querySelector(
          this.attributionBubbleToggleSelector,
        );
      // Define the dialog element within the wrapping div
      const attributionBubbleDialogElement =
        attributionBubbleElement.querySelector(
          this.attributionBubbleDialogSelector,
        );

      // Abort if the event or any of the elements was not found
      if (
        !event ||
        !attributionBubbleElement ||
        !attributionBubbleToggleElement ||
        !attributionBubbleDialogElement
      ) {
        return;
      }
    ...
    attributionBubbleBackdrop(event) {
      // Abort if no event was found
      if (!event) {
        return;
      }
      ...
      const isBackdrop =
        !event.target.closest(this.attributionBubbleToggleSelector) &&
        !event.target.closest(this.attributionBubbleDialogSelector);
      if (isBackdrop) {
    ...
    attributionBubbleKeypress(event) {
      if (!event || !event.key) {
        return;
      }
    ...
    attributionBubbleCleanup() {
      // Close all open dialogs
      document
        .querySelectorAll(`${this.attributionBubbleDialogSelector}[open]`)
        .forEach((attributionBubbleDialogElement) => {
          attributionBubbleDialogElement.close();
        });
      ...
      document
        .querySelectorAll(
          `${this.attributionBubbleToggleSelector}[data-active]`,
        )
        .forEach((attributionBubbleToggleElement) => {
          delete attributionBubbleToggleElement.dataset.active;
        });
    },
  };
