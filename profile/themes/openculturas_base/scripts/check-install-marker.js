...
const installedHash = fs.existsSync(markerFilePath) ? fs.readFileSync(markerFilePath, 'utf8').trim() : null;

if (currentHash !== installedHash) {
  console.error('\npackage-lock.json has changed since the last `npm ci` / `npm install` in this theme.');
  console.error('Run `npm ci` in profile/themes/openculturas_base, then try the build again.\n');
  process.exit(1);
}
